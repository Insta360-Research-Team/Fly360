...
parser.add_argument('--resume', default='weights/fly360.pth')
parser.add_argument('--env', default='defult')
parser.add_argument('--max_speed', default=10.0, type=float, help='(m/s) real speed might be 2 m/s slower')
...
agents = {
    "defult": [
        ("SimpleFlight", [[-0, -1.5, -1], [50, -1.5, -1],[-0, -1.5, -1], [50, -1.5, -1],[-0, -1.5, -1], [50, -1.5, -1],[-0, -1.5, -1], [50, -1.5, -1]]),  
    ],
}[args.env]
...
def main():
    ...
    while t_now < t_end:
        ...
        erp_depth = torch.from_numpy(img_pd[:, :, 0]).float().to(device).unsqueeze(0).unsqueeze(0) / 100.0
        ...
        # ===== visulization =====
        depth_np = erp_depth[0, 0].detach().cpu().numpy().astype(np.float32)  # (H,W)
        ...
        state_cat = torch.cat(state_vecs, dim=-1).to(device)     # (B, total_dim)

    
        depth_t = torch.from_numpy(depth_norm).float().to(device)   # (B, 6, 224, 224)
        depth_t = depth_t.unsqueeze(0)


        d_min, d_max = 0.01, 50.0
        depth_t = depth_t * (d_max - d_min) + d_min
        
        depth_t = 3.0 / depth_t.clamp_(0.3, 24.0) - 0.6
        depth_t = depth_t.unsqueeze(0)
        depth_t = F.interpolate(depth_t, size=(64, 128), mode='bilinear', align_corners=False)  # (1,6,48,36)
        ...
        for i, (r, p, y, t) in enumerate(zip(roll.tolist(), pitch.tolist(), yaw.tolist(), throttle.tolist())):
            # if done_flag[i]:
            #     continue
            t = t / 9.8 * hover_thr  # 归一化推力
            y = normalize_angle(y)
            print(f"Final throttle for {agent_names[i]}: {t}")
            client.moveByRollPitchYawThrottleAsync(r, p, y, t, 0.1)
